...
from core.mypool.Mypool import Mythreadingpool  # the threading pool
from conf.config_server import DATA_DIR,REQUEST_Q_SIZE
import traceback
import shutil
...
class MYTCPServer:
	...
	coding='utf-8'
	request_queue_size = REQUEST_Q_SIZE
	server_dir='file_upload'
	operation_list = ['put', 'get', 'mkdir', 'cd', 'ls', 'rm','vol']  # 限制用户的指令

	def __init__(self, server_address, bind_and_activate=True, pool_size=10):
		"""Constructor.  May be extended, do not override."""
		self.server_pool = Mythreadingpool(pool_size)
	...
	def run(self):
		while True:
			...
			self.server_pool.submit(self.waiter, (conn,addr,))
